export const DEFAULT_PUBLIC_ORIGIN = "https://www.dgeniussolutions.com";
export const DEFAULT_WP_ORIGIN = "https://wp-origin.dgeniussolutions.com";

export function getWordpressBackendOrigin() {
  return (
    process.env.DGS_WORDPRESS_BACKEND_ORIGIN ||
    process.env.WORDPRESS_BACKEND_ORIGIN ||
    DEFAULT_WP_ORIGIN
  ).replace(/\/+$/, "");
}

export function getPublicSiteOrigin() {
  return (
    process.env.NEXT_PUBLIC_SITE_URL ||
    DEFAULT_PUBLIC_ORIGIN
  ).replace(/\/+$/, "");
}

function noncePatterns(formId) {
  const name = `_fluentform_${formId}_fluentformnonce`;
  return [
    new RegExp(`name=["']${name}["'][^>]*value=["']([^"']+)["']`, "i"),
    new RegExp(`value=["']([^"']+)["'][^>]*name=["']${name}["']`, "i"),
  ];
}

function extractNonce(html, formId) {
  for (const pattern of noncePatterns(formId)) {
    const match = html.match(pattern);
    if (match?.[1]) return match[1];
  }
  return "";
}

function extractScopedEmbeddedPostId(html, formId) {
  const wrapperPatterns = [
    new RegExp(
      `fluentform_wrapper_${formId}[\\s\\S]{0,12000}?name=["']__fluent_form_embded_post_id["'][^>]*value=["'](\\d+)["']`,
      "i",
    ),
    new RegExp(
      `fluentform_wrapper_${formId}[\\s\\S]{0,12000}?value=["'](\\d+)["'][^>]*name=["']__fluent_form_embded_post_id["']`,
      "i",
    ),
    new RegExp(
      `data-form_id=["']${formId}["'][\\s\\S]{0,12000}?name=["']__fluent_form_embded_post_id["'][^>]*value=["'](\\d+)["']`,
      "i",
    ),
    new RegExp(
      `data-form_id=["']${formId}["'][\\s\\S]{0,12000}?value=["'](\\d+)["'][^>]*name=["']__fluent_form_embded_post_id["']`,
      "i",
    ),
  ];
  for (const pattern of wrapperPatterns) {
    const match = html.match(pattern);
    if (match?.[1]) return match[1];
  }
  return "";
}

/**
 * Resolve Fluent Forms nonce + embedded post ID for a single route/form pair.
 * Never borrows homepage Form 1 context for another form.
 */
export function resolveFormContextFromHtml(html, definition, route, publicOrigin) {
  const formId = Number(definition.fluentFormId);
  const routePageId = definition.backend?.wordpressPageIds?.[route];
  const origin = publicOrigin || getPublicSiteOrigin();
  const referer = `${origin}${route}`;
  const nonce = extractNonce(html, formId);

  if (!nonce) {
    return {
      ok: false,
      message: "Form security token unavailable for this route",
      status: 502,
      referer,
      routePageId: routePageId ? String(routePageId) : "",
    };
  }

  const scopedPostId = extractScopedEmbeddedPostId(html, formId);
  const embeddedPostId = scopedPostId || (routePageId ? String(routePageId) : "");

  if (!embeddedPostId) {
    return {
      ok: false,
      message: "Form page context unavailable for this route",
      status: 502,
      referer,
      routePageId: routePageId ? String(routePageId) : "",
    };
  }

  return {
    ok: true,
    nonce,
    embeddedPostId,
    referer,
    routePageId: String(routePageId || embeddedPostId),
  };
}

const CONTEXT_CACHE = new Map();
const CONTEXT_CACHE_TTL_MS = 5 * 60 * 1000; // 5 minutes

export function clearFormContextCache(formId, route) {
  if (formId && route) {
    CONTEXT_CACHE.delete(`${formId}:${route}`);
  } else if (formId) {
    for (const key of CONTEXT_CACHE.keys()) {
      if (key.startsWith(`${formId}:`)) CONTEXT_CACHE.delete(key);
    }
  } else {
    CONTEXT_CACHE.clear();
  }
}

export async function fetchFormContext(definition, route, fetchImpl = fetch, bypassCache = false) {
  const formId = Number(definition?.fluentFormId);
  const routePageId = definition.backend?.wordpressPageIds?.[route];
  const cacheKey = `${formId}:${route}`;
  const now = Date.now();

  if (!bypassCache && CONTEXT_CACHE.has(cacheKey)) {
    const cached = CONTEXT_CACHE.get(cacheKey);
    if (cached && now - cached.timestamp < CONTEXT_CACHE_TTL_MS) return cached.context;
    CONTEXT_CACHE.delete(cacheKey);
  }

  const backendOrigin = getWordpressBackendOrigin();
  const publicOrigin = getPublicSiteOrigin();
  const publicReferer = `${publicOrigin}${route}`;
  let html = '';
  const apiUrl = new URL('/wp-json/dgs/v1/form-context', backendOrigin);
  apiUrl.searchParams.set('form_id', String(formId));
  if (routePageId) apiUrl.searchParams.set('post_id', String(routePageId));

  try {
    const response = await fetchImpl(apiUrl, {
      headers: { 'User-Agent': 'DGS-Form-Submit/Headless', Accept: 'application/json' },
      cache: 'no-store',
    });
    if (response.ok) {
      const payload = await response.json();
      html = typeof payload?.html === 'string' ? payload.html : '';
    }
  } catch {}

  if (!html) {
    const legacyUrl = `${backendOrigin}${route}`;
    try {
      const response = await fetchImpl(legacyUrl, {
        headers: { 'User-Agent': 'DGS-Form-Submit/Legacy-Fallback', Accept: 'text/html' },
        cache: 'no-store',
      });
      if (!response.ok) throw new Error('legacy context unavailable');
      html = await response.text();
    } catch {
      return { ok: false, message: 'Unable to load form context', status: 502, referer: publicReferer,
        routePageId: routePageId ? String(routePageId) : '' };
    }
  }

  const result = resolveFormContextFromHtml(html, definition, route, publicOrigin);
  if (result.ok) CONTEXT_CACHE.set(cacheKey, { timestamp: now, context: result });
  return result;
}
