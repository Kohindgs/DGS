...
function fieldStringValue(raw, field) {
  if (Array.isArray(raw)) return raw.map(String).join(",");
  if (raw === true) return "1";
  if (raw === false) return "";
  if (raw == null) return "";
  const str = String(raw);
  if (field.type === "checkbox" && !field.options?.length && (str === "on" || str === "1" || str === "true")) {
    return "1";
  }
  if (field.type === "select" || field.type === "radio") return str;
  return str.trim();
}
...
export function validateFormPayload(input, { getDefinitionById, assertRouteFormMapping }) {
  ...
  for (const field of definition.fields || []) {
    ...
    const value = fieldStringValue(raw, field);
    ...
    if (field.type === "checkbox" && !field.options?.length && value !== "1") {
      fieldErrors[field.name] = `Invalid value for ${field.label}`;
      continue;
    }
