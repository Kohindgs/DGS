...
export function prepareWpHtml(html = '') {
  ...
  // Soften embedded <style> so WP pages stay fluid from 4K → phones.
  out = out.replace(/<style\b([^>]*)>([\s\S]*?)<\/style>/gi, (_, attrs, css) => {
    let next = css.replace(
      /@media\s*\(\s*min-width\s*:\s*(?:2000|2560|3000|3200)px\s*\)\s*\{(?:[^{}]|\{[^{}]*\})*\}/gi,
      ''
    );
    next = next.replace(/\bmin-width\s*:\s*(7\d{2}|[89]\d{2}|1\d{3,4})px/gi, 'min-width: 0px');
    next = next.replace(/\bwidth\s*:\s*(1[4-9]\d{2}|[2-9]\d{3})px/gi, 'width: 100%');
...
export function wpMetadata(item, { fallbackTitle, fallbackDescription, path = '/' } = {}) {
  ...
  return {
    title, // layout template appends " | D'Genius Solutions"
    description,
...
export const RESERVED_SLUGS = new Set(['blogs', 'services', 'api', '_next', 'about-us']);
