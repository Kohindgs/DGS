...
/**
 * Envira often ships only a 1024px asset labeled as 400w/2x. Prefer WP mid-sizes
 * (~300 / ~768) in src/srcset so tiles are not multi‑MB downloads for ~100px CSS.
 * Original URL is kept in data-dgs-full-src for client fallback on 404.
 */
export function compactEnviraImageSources(html = '') {
  const scaleUrl = (url, targetW) => {
    const m = String(url).match(/^(.*)-(\d+)x(\d+)(\.(?:png|jpe?g|webp))(\.webp)?(\?[^"']*)?$/i);
    if (!m) return null;
    const srcW = Number(m[2]);
    const srcH = Number(m[3]);
    if (!srcW || srcW <= targetW) return null;
    const nh = Math.max(1, Math.round((srcH / srcW) * targetW));
    return `${m[1]}-${targetW}x${nh}${m[4]}${m[5] || ''}${m[6] || ''}`;
  };

  return html.replace(/<img\b([^>]*\benvira-gallery-image[^>]*)>/gi, (full, attrs) => {
    const get = (name) => {
      const m = attrs.match(new RegExp(`\\s${name}=["']([^"']*)["']`, 'i'));
      return m ? m[1] : null;
    };
    const src = get('src') || get('data-envira-src') || get('data-src') || '';
    if (!src || isPlaceholderSrc(src)) return full;
    const dim = src.match(/-(\d+)x(\d+)\./);
    if (!dim || Number(dim[1]) < 640) return full;

    const fullSrc = src;
    const src300 = scaleUrl(src, 300);
    const src768 = scaleUrl(src, 768);
    if (!src300) return full;

    const origW = Number(dim[1]);
    const candidates = [];
    if (src300) candidates.push(`${src300} 300w`);
    if (src768) candidates.push(`${src768} 768w`);
    candidates.push(`${fullSrc} ${origW}w`);
    ...
    setAttr('sizes', '(max-width: 900px) 46vw, 240px');
    setAttr('data-dgs-full-src', fullSrc);
    // Keep Envira lazy attrs aligned with the compact src.
    if (/\sdata-envira-src=/i.test(a)) setAttr('data-envira-src', src300);
    if (/\sdata-src=/i.test(a)) setAttr('data-src', src300);
    if (/\sdata-envira-srcset=/i.test(a)) setAttr('data-envira-srcset', candidates.join(', '));
 ...
 */
export function applyPerfImageHints(html = '') {
...
/**
 * Point the hero robot at a WP mid-size variant so mobile LCP is not a
 * 2000×2000 / ~380KB download. Single src (no srcset) avoids double-fetch.
 */
export function optimizeHeroRobotImage(html = '', demoOrigin = DEMO_ORIGIN) {
  return html.replace(/<img\b([^>]*\bid=["']dgs-v1215-robot["'][^>]*)>/i, (full, attrs) => {
    const srcMatch = attrs.match(/\bsrc=["']([^"']+)["']/i);
    if (!srcMatch) return full;
    const src = srcMatch[1];
    const m = src.match(
      /^(.*\/thoughtful-logo-concept-featuring-ai-meaningful-way)(?:-\d+x\d+)?(\.webp)(\?[^"']*)?$/i
    );
    if (!m) return full;

    const src1024 = `${m[1]}-1024x1024${m[2]}${m[3] || ''}`;
    let a = attrs;
    a = a
      .replace(/\ssrc=["'][^"']*["']/i, ` src="${src1024}"`)
      .replace(/\ssrcset=["'][^"']*["']/gi, '')
      .replace(/\ssizes=["'][^"']*["']/gi, '')
      .replace(/\swidth=["'][^"']*["']/gi, '')
      .replace(/\sheight=["'][^"']*["']/gi, '');
...
export function relocateHeroAboutSection(html = '', demoOrigin = DEMO_ORIGIN) {
  ...
  const coverSrc = '/media/dgs-anthem-cover-1280.webp';
  const coverSrcset =
    '/media/dgs-anthem-cover-960.webp 960w, /media/dgs-anthem-cover-1280.webp 1280w, /media/dgs-anthem-cover-1600.webp 1600w';
...
export async function getWpHomeMirror({ revalidate = 300 } = {}) {
  ...
  const lcpImage = extractLcpImageUrl(bodyHtml);
