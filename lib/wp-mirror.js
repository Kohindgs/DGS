...
function patchPortfolioScript(code = '') {
  ...
  if (/__DGS_AI_VIDEO_PORTFOLIO__/.test(next)) {
    ...
    next = next.replace(/var itemsPerLoad=[^;]+;/, 'var itemsPerLoad=6;');
    // Only cap category-tab renders — do NOT slice when building allVideos.
    next = next.replace(
      /function renderCat\(k\)\{if\(k==='all'\)return;var c=document\.getElementById\(k\+'-gallery'\);if\(!c\|\|c\.children\.length\|\|!portfolioData\[k\]\)return;var f=document\.createDocumentFragment\(\);portfolioData\[k\]\.forEach\(function\(v\)\{/,
      "function renderCat(k){if(k==='all')return;var c=document.getElementById(k+'-gallery');if(!c||c.children.length||!portfolioData[k])return;var f=document.createDocumentFragment();portfolioData[k].slice(0,6).forEach(function(v){"
    );
    // Undo accidental allVideos builder slice from earlier deploys.
    next = next.replace(
      /Object\.keys\(portfolioData\)\.forEach\(function\(k\)\{portfolioData\[k\]\.slice\(0,6\)\.forEach\(function\(v\)\{/,
      'Object.keys(portfolioData).forEach(function(k){portfolioData[k].forEach(function(v){'
    );
  }
...
/** Wait for Three.js, then defer heavy particle-sphere until after first paint. */
function patchWebglBootScript(code = '') {
  ...
  // Skip on mobile / reduced-motion — keeps first paint snappy.
  next = next.replace(
    /var container=document\.getElementById\('global-webgl-background'\);if\(!container\|\|!window\.THREE\)return;/,
    "var container=document.getElementById('global-webgl-background');if(!container||container.dataset.dgsWebgl==='1')return;try{if(window.matchMedia&&(window.matchMedia('(max-width:900px)').matches||window.matchMedia('(prefers-reduced-motion:reduce)').matches||window.matchMedia('(pointer:coarse)').matches))return;}catch(e){}if(!window.THREE){if(window.__dgsOnThree)return void window.__dgsOnThree(dgsWebglRun);var n=0,t=setInterval(function(){if(window.THREE){clearInterval(t);dgsWebglRun();}else if(++n>100)clearInterval(t);},50);return;}container.dataset.dgsWebgl='1';"
  );
  // 5200 particles freezes mid-tier CPUs — keep the sphere look with fewer points.
  next = next.replace(/var particleCount=5200/, 'var particleCount=700');
  next = next.replace(
    /renderer\.setPixelRatio\(Math\.min\(window\.devicePixelRatio\|\|1,2\)\)/g,
    'renderer.setPixelRatio(1)'
  );
  // Defer start until after paint (do not call dgsWebglRun synchronously).
  next = next.replace(
    /\}\)\(\)\s*;?\s*$/,
    '}function dgsWebglStart(){if(window.requestIdleCallback)requestIdleCallback(function(){dgsWebglRun()}, {timeout:2500});else setTimeout(dgsWebglRun,1200);}if(document.readyState===\'complete\')dgsWebglStart();else window.addEventListener(\'load\',dgsWebglStart,{once:!0});})();'
  );
  return next;
}
...
export async function getWpPageMirror({
  ...
  // Drop leftover WP stylesheet <link>s — we already ship one CSS bundle.
  bodyHtml = bodyHtml.replace(/<link\b[^>]*rel=["'][^"']*stylesheet[^"']*["'][^>]*>/gi, '');
  bodyHtml = bodyHtml.replace(/<link\b[^>]*href=["'][^"']*\.css(\?[^"']*)?["'][^>]*>/gi, '');
