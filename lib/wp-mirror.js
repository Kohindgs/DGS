...
function patchPortfolioScript(code = '') {
  ...
  // the time DeferredHomeClient injects the script, so cards + FAQ never bind.
  if (/__DGS_AI_VIDEO_PORTFOLIO__/.test(next) && /DOMContentLoaded/.test(next)) {
    next = next.replace(
      ...
      'function __dgsAiVideoBoot(){'
    );
    next = next.replace(
      /obs\.observe\(s\)\}\)\}\)\}\)\(\)\s*;?\s*$/,
      "obs.observe(s)});if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',__dgsAiVideoBoot);else __dgsAiVideoBoot();}})()"
    );
  }
