...
/** Rewrite absolute WP asset urls in CSS so fonts/icons load same-origin via Next rewrites. */
export function rewriteWpUrlsInCss(css = '') {
  return css
    .replace(
...
export async function getWpHomeMirror({ revalidate = 300 } = {}) {
  ...
  const softenedStyles = inlineStyles.map((css) => {
    ...
    return rewriteWpUrlsInCss(softened);
  });
