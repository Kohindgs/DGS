...
export function optimizeHeroRobotImage(html = '', demoOrigin = DEMO_ORIGIN) {
  return html.replace(/<img\b([^>]*\bid=["']dgs-v1215-robot["'][^>]*)>/i, (full, attrs) => {
    ...
    let a = attrs;
    // Default src = 640 (mobile LCP). Desktop can still pick 720/800 via srcset.
    a = a
...
function isCriticalInline(code = '') {
  ...
  return (
    /dgsToggle|dgsSvc|dgsLockScroll|dgsOpenTalkPopup|dgsNav|__DGS_PORTFOLIO|portfolio-lightbox|dgsLogo|portfolioData|initPortfolio|bootMotion|envira_gallery|fluentFormVars|fluent_form_ff_form_instance|fluentformElementor/.test(
      code
    ) &&
