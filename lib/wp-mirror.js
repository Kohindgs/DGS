...
function patchPortfolioScript(code = '') {
  ...
  if (/__DGS_AI_VIDEO_PORTFOLIO__/.test(next)) {
    ...
    // Guard: never append more than 6 until the user clicks Load More.
    next = next.replace(
      /window\.__dgsAiVideoBoot=function\(\)\{loadMore\(\);/,
      "window.__dgsAiVideoBoot=function(){currentlyDisplayed=0;var g0=document.getElementById('portfolio-gallery');if(g0)g0.innerHTML='';loadMore();"
    );
  }
...
function patchWebglBootScript(code = '') {
  ...
  next = next.replace(/var particleCount=5200/, 'var particleCount=1400');
