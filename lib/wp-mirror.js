...
export function toBundledStylesheet(_hrefs = []) {
  return '/api/wp-css?bundle=home&v=08i';
}
...
export function compactEnviraImageSources(html = '') {
  ...
  return html.replace(/<img\b([^>]*)>/gi, (full, attrs) => {
    ...
    // Logo wall tiles paint ~230px; case screenshots ~half viewport; gallery ~240px.
    const w1 = isLogoTile ? 230 : isCaseMedia || isScaled ? 640 : 240;
    const w2 = isLogoTile ? 460 : isCaseMedia || isScaled ? 960 : 480;
    const srcSmall = withWidth(fullSrc, w1);
    const srcMid = withWidth(fullSrc, w2);
    if (!srcSmall || !srcMid) return full;

    const origW = srcW || (isScaled ? 1600 : 1024);
    const candidates = [`${srcSmall} ${w1}w`, `${srcMid} ${w2}w`, `${fullSrc} ${origW}w`];
    ...
    setAttr(
      ...
      isLogoTile
        ...
        : isCaseMedia || isScaled
          ...
          : isTile
            ? '(max-width: 900px) 46vw, 240px'
            : '(max-width: 900px) 40vw, 180px'
    );
    setAttr('data-dgs-full-src', fullSrc);
    if (/\sdata-envira-src=/i.test(a)) setAttr('data-envira-src', srcSmall);
...
export function compactContainerImages(html = '') {
  ...
  const rewriteImg = (imgTag, w1, w2, sizes) =>
    imgTag.replace(/<img\b([^>]*)>/i, (full, attrs) => {
      ...
      setAttr('src', s1);
      setAttr('srcset', `${s1} ${w1}w, ${s2} ${w2}w, ${fullSrc} ${Math.max(w2 * 2, 1200)}w`);
      setAttr('sizes', sizes);
      setAttr('data-dgs-full-src', fullSrc);
      return `<img${a}>`;
    });
...
/**
 * Point the hero robot at a sharp-resized proxy URL. Display size stays the
 * same via CSS (~360–500px); we only download ~640–800px instead of 1024+.
 */
export function optimizeHeroRobotImage(html = '', demoOrigin = DEMO_ORIGIN) {
  return html.replace(/<img\b([^>]*\bid=["']dgs-v1215-robot["'][^>]*)>/i, (full, attrs) => {
    ...
    const src = srcMatch[1].replace(/([?&])dgs_w=\d+/g, '').replace(/[?&]$/, '');
    const m = src.match(
      /^(.*\/thoughtful-logo-concept-featuring-ai-meaningful-way)(?:-\d+x\d+)?(\.webp)(\?[^"']*)?$/i
    );
    // Prefer the 1024 master as resize input when the path matches.
    const master = m ? `${m[1]}-1024x1024${m[2]}${m[3] || ''}` : src;
    const join = master.includes('?') ? '&' : '?';
    const src640 = `${master}${join}dgs_w=640`;
    const src720 = `${master}${join}dgs_w=720`;
    const src800 = `${master}${join}dgs_w=800`;

    let a = attrs;
    a = a
      .replace(/\ssrc=["'][^"']*["']/i, ` src="${src720}"`)
      .replace(/\ssrcset=["'][^"']*["']/gi, '')
      .replace(/\ssizes=["'][^"']*["']/gi, '')
      .replace(/\swidth=["'][^"']*["']/gi, '')
      .replace(/\sheight=["'][^"']*["']/gi, '')
    ...
    a += ' sizes="(max-width: 900px) min(92vw, 420px), 500px"';
