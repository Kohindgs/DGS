...
export async function getWpHomeMirror({ revalidate = 300 } = {}) {
  ...
  const lcpImage = getHeroRobotLcpPreload().href || extractLcpImageUrl(bodyHtml);
  ...
  return {
    ...
    stylesheets,
    // WP custom CSS (nav #dgsNav/#dgsTrig, .dgs-v1215 theme) lives in <style> tags,
    // not in linked stylesheets — the home CSS bundle cannot replace these.
