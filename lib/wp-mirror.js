...
/**
 * Portfolio: do NOT invent missing -thumb.webp (half 404) and do NOT preload
 * every MP4 — that stampeded Hostinger and starved case-study image requests.
 * Posters only when video.thumb is set; videos lazy-load via client IO.
 */
function patchPortfolioScript(code = '') {
  ...
  // Disable auto thumb URL invention (many files do not exist on WP).
  next = next.replace(/var\s+AUTO_THUMBS\s*=\s*!0\b/, 'var AUTO_THUMBS=!1');
  next = next.replace(/var\s+AUTO_THUMBS\s*=\s*!1\b/, 'var AUTO_THUMBS=!1');
  next = next.replace(/AUTO_THUMBS\s*=\s*!0\b/g, 'AUTO_THUMBS=!1');
  next = next.replace(/AUTO_THUMBS\s*=\s*!1\b/g, 'AUTO_THUMBS=!1');
  ...
  // Keep optional poster img only when an explicit thumb exists (AUTO_THUMBS off).
...
function patchMotionScript(code = '') {
  ...
  // Particles already orbit inside the canvas; the plane itself must stay fixed.
  next = next.replace(
    /root\.style\.setProperty\(["']--v1215-grid-x["'][^)]*\)/g,
    'root.style.setProperty("--v1215-grid-x","0px")'
  );
  next = next.replace(
    /root\.style\.setProperty\(["']--v1215-grid-y["'][^)]*\)/g,
