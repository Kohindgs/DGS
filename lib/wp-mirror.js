...
export function compactEnviraImageSources(html = '') {
  ...
  return html.replace(/<img\b([^>]*)>/gi, (full, attrs) => {
    ...
    const shouldCompact =
      isTile || srcW >= 400 || (/smush-webp/i.test(src) && srcW >= 360);
    if (!shouldCompact) return full;

    const fullSrc = stripWidth(src);
    const src240 = withWidth(fullSrc, 240);
    const src480 = withWidth(fullSrc, 480);
    if (!src240 || !src480) return full;

    const origW = srcW || 1024;
    const candidates = [`${src240} 240w`, `${src480} 480w`, `${fullSrc} ${origW}w`];
    ...
    setAttr('sizes', isTile ? '(max-width: 900px) 46vw, 240px' : '(max-width: 900px) 40vw, 180px');
    setAttr('data-dgs-full-src', fullSrc);
    if (/\sdata-envira-src=/i.test(a)) setAttr('data-envira-src', src240);
    if (/\sdata-src=/i.test(a)) setAttr('data-src', src240);
