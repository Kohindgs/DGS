...
export function toBundledStylesheet(_hrefs = [], bundle = 'home') {
  const safe = bundle === 'about' ? 'about' : 'home';
...
function isCriticalInline(code = '') {
  ...
  return (
    /dgsToggle|dgsSvc|dgsLockScroll|dgsOpenTalkPopup|dgsNav|__DGS_PORTFOLIO|portfolio-lightbox|dgsLogo|portfolioData|initPortfolio|bootMotion|envira_gallery|fluentFormVars|fluent_form_ff_form_instance|fluentformElementor|dgsRunAiPrompt|dgsAiPromptText|dgsShowAiPromptToast/.test(
      code
    ) &&
...
export async function getWpPageMirror({
  ...
  bodyHtml = applyPerfImageHints(bodyHtml, {
    prioritizeFirstUpload: mode === 'about',
  });
  ...
  const canonical = `${DEMO_ORIGIN.replace(/\/$/, '')}${demoPath === '/' ? '/' : demoPath}`;
