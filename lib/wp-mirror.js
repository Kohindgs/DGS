...
export function applyPerfImageHints(html = '') {
  ...
  return html.replace(/<img\b([^>]*)>/gi, (_full, attrs) => {
    ...
    const isEnvira = /\benvira-gallery-image\b/.test(cls);
    const isLogo =
      id === 'dgsLogo' ||
    ...
    if (isRobot) {
      ...
      enviraIdx += 1;
      // First tiles drive justified layout; the rest wait for scroll.
      a += enviraIdx <= 8 ? ' loading="eager" decoding="async"' : ' loading="lazy" decoding="async"';
    } else if (isLogo) {
...
/**
 * Point the hero robot at WP-generated size variants so mobile LCP is not a
 * 2000×2000 / ~380KB download. Visual stays the same; browser picks by layout.
 */
export function optimizeHeroRobotImage(html = '', demoOrigin = DEMO_ORIGIN) {
  return html.replace(/<img\b([^>]*\bid=["']dgs-v1215-robot["'][^>]*)>/i, (full, attrs) => {
    ...
    // Expect .../thoughtful-logo-....webp (optionally already proxied)
    const m = src.match(/^(.*\/thoughtful-logo-concept-featuring-ai-meaningful-way)(?:-\d+x\d+)?(\.webp)(\?[^"']*)?$/i);
    if (!m) return full;

    const base = m[1];
    const ext = m[2];
    const qs = m[3] || '';
    const u = (size) => `${base}${size}${ext}${qs}`;

    const src768 = u('-768x768');
    const src1024 = u('-1024x1024');
    const src1536 = u('-1536x1536');
    const srcFull = u('');
    ...
    a = a
      ...
      .replace(/\swidth=["'][^"']*["']/gi, '')
      .replace(/\sheight=["'][^"']*["']/gi, '');
    a += ` width="1024" height="1024" srcset="${src768} 768w, ${src1024} 1024w, ${src1536} 1536w, ${srcFull} 2000w" sizes="(max-width: 900px) min(92vw, 420px), min(42vw, 560px)"`;
    return `<img${a}>`;
  });
}
