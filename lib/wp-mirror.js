 ...
 * Also soften video preload (avoid connection stampede) and recover when thumbs 404.
 */
function patchPortfolioScript(code = '') {
  ...
  // Prefer metadata over auto — 40× full video preload was killing connections.
  next = next.replace(/preload="auto"/g, 'preload="metadata"');
  next = next.replace(/preload=\\"auto\\"/g, 'preload=\\"metadata\\"');
  next = next.replace(/preload:\\"auto\\"/g, 'preload:\\"metadata\\"');
  next = next.replace(/preload:'auto'/g, "preload:'metadata'");
