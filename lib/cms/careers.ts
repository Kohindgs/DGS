...
export type CmsCareerJob = {
  ...
  active: number | boolean; created_at?: string; updated_at?: string;
};
...
export async function createCmsCareerJob(input: Omit<CmsCareerJob,"id"|"created_at"|"updated_at">) {
  ...
  await cmsExecute(
    ...
    (id,slug,title,summary,employment_type,employment_label,location,workplace_type,schedule,experience,compensation,overview,responsibilities,requirements,benefits,date_posted,active)
    VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`,
    [id,input.slug,input.title,input.summary,input.employment_type,input.employment_label,input.location,input.workplace_type,input.schedule,input.experience,input.compensation,input.overview,input.responsibilities,input.requirements,input.benefits,input.date_posted,Boolean(input.active)],
  );
  return id;
}
