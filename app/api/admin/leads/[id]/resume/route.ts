...
export async function GET(_request: Request,{ params }:{ params:Promise<{id:string}> }) {
  ...
  const resume = payload.resume as { storedName?:string; originalName?:string; mimeType?:string } | undefined;
  if (!resume?.storedName) return NextResponse.json({ ok:false, message:"No CV attached" },{ status:404 });

  const uploadRoot = process.env.DGS_PRIVATE_UPLOAD_DIR || path.join(process.cwd(),"storage","careers");
  const safeStoredName = path.basename(resume.storedName);
  if (safeStoredName !== resume.storedName) return NextResponse.json({ ok:false },{ status:400 });
  ...
  try {
    ...
    return new NextResponse(new Uint8Array(file),{
      headers:{
        ...
        "Content-Disposition":`attachment; filename="${(resume.originalName || "candidate-cv").replace(/"/g,"")}"`,
        "Cache-Control":"private, no-store",
      },
    });
  } catch {
    return NextResponse.json({ ok:false, message:"CV file not found" },{ status:404 });
  }
}
