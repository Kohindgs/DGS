...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  ...
  const entries: MetadataRoute.Sitemap = routes
    ...
    .map((route) => {
      ...
      const url = canonicalPath.startsWith("http") ? canonicalPath : `${siteConfig.url}${canonicalPath}`;
      const entry: MetadataRoute.Sitemap[number] = { url };
      if (route.modified) entry.lastModified = route.modified;
      return entry;
    });
  ...
  if (isCmsDatabaseConfigured()) {
    try {
      ...
      for (const blog of await listPublishedCmsBlogs()) {
        ...
        if (!existing.has(url.replace(/\/$/, ""))) entries.push({ url, lastModified: blog.updated_at });
      }
    } catch {
